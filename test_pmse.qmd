...
map2(list(XR3, XR10, XR20),
     list(XS3, XS10, XS20),
     function(real, synthetic) {
       D <- rbind(real, synthetic) |> scale()
       USV <- svd(D)
       D %*% USV$v |>
         matrix(ncol = ncol(D), dimnames = list(NULL, paste0("Dim", 1:ncol(D)))) |>
         data.frame() |>
         mutate(Synthetic = rep(c("Real", "Synthetic"), each = nrow(D)/2)) |>
         ggplot(aes(Dim1, Dim2, col = Synthetic)) +
         geom_point() +
         theme_minimal() +
...
map2(list(XR3, XR10, XR20),
     ...
     function(real, synthetic) {
       ...
       R <- real %*% USV$v
       D <- synthetic %*% USV$v
       synthpop::utility.gen(data.frame(R[,1:2]),
                             data.frame(D[,1:2]),
                             method = "logit",
                             max.order = 2)
})
